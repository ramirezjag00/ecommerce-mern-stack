...
export default function (state = {
  ...
  switch (action.type) {
    ...
    // create a copy of the current array of ITEMS
      ...
      const indexToDelete = currentItemToDelete.findIndex(item => (
        item._id.toString() === action.payload
      ));
      // use slice to remove the item at the specified index
      return {
        items: [...currentItemToDelete.slice(0, indexToDelete),
    ...
    // create a copy of the current array of items
      ...
      const indexToUpdate = currentItemToUpdate.findIndex(item => (
      ...
      return {
        items: [...currentItemToUpdate.slice(0, indexToUpdate), newItemToUpdate,
